...
description: タスク完了時に「最初に失敗した内容」と「最終的に通った解法」を対応付け、最初に知っておくべきだった知見を ast-grep ルール / skill / AGENTS.md ルールのいずれかに言語化する。試行錯誤の末にたどり着いた解や、同じ落とし穴を将来の自分（または別エージェント）に繰り返させたくないときに使う。ユーザーから「今回の学びをルール化して」「skill にして」「lint に落として」と指示されたとき、またはタスク終了時に学びを棚卸しする場面で起動する。
---
...
4. **重複チェック（必須）**: 提案前に既存の知見と照合する。重複や近接する規則があれば「新規追加」ではなく「既存への追記 / 更新」を選ぶ。これを怠ると skill / ルールが肥大化する。
   ...
   ```
   # skill 重複（global）
   ls ~/.local/share/chezmoi/dot_config/opencode/skills/
   Grep "<キー>" ~/.local/share/chezmoi/dot_config/opencode/skills/*/SKILL.md

   # AGENTS.md 重複
   Grep "<キー>" ~/.local/share/chezmoi/dot_config/opencode/AGENTS.md
...
分類は、将来頻度や主観的な「毎回使う感じ」で決めない。次の観測可能な軸で決める。

- **静的検出可能性**: コードや設定だけを見て機械的に検出できるか
- **適用範囲**: 言語横断・ツール横断の一般則か、特定リポジトリ限定か、一回限りか
- **手順性**: 実行に複数ステップや文脈判断が必要か
- **既存 artifact 依存**: 既存 skill や要件文書へ書き戻す前提があるか

判定に迷うときは、頻度を推測して決めず、上の 4 軸に照らして一段保守的な出力先を選ぶ。

```dot
digraph classify {
    "機械的に検出可能？" [shape=diamond];
    "言語横断・ツール横断の短い一般則か？" [shape=diamond];
    "複数ステップの手順や文脈判断を伴うか？" [shape=diamond];
    ...
    "機械的に検出可能？" -> "言語横断・ツール横断の短い一般則か？" [label="no"];
    "言語横断・ツール横断の短い一般則か？" -> "AGENTS.md ルール" [label="yes"];
    "言語横断・ツール横断の短い一般則か？" -> "複数ステップの手順や文脈判断を伴うか？" [label="no"];
    "複数ステップの手順や文脈判断を伴うか？" -> "skill" [label="yes"];
    "複数ステップの手順や文脈判断を伴うか？" -> "メモに留める" [label="no"];
}
```
...
| 言語横断・ツール横断の短い一般則で、文脈判断をほぼ要しない | `AGENTS.md`（user global / project） | "pnpm は v10 以上を使う" |
| 手順・文脈判断・テンプレが必要 | 新規 skill または既存 skill への追記 | "MoonBit の C binding を書く手順" |
| 適用範囲が単一タスクに閉じ、将来の artifact へ一般化できない | 採用しない（コミットメッセージ / PR 説明に留める） | — |

**ast-grep を優先する原則**: 静的に検出可能なものはプロンプトやドキュメントに書かず、必ず `ast-grep` ルールにする（ユーザーの global ルール）。

**AGENTS.md の書き出し先**:
- 言語横断・ツール横断の一般則 → `~/.local/share/chezmoi/dot_config/opencode/AGENTS.md`
...
→ `~/.local/share/chezmoi/dot_config/opencode/AGENTS.md` の「ツール」節に追記:
```markdown
...
重複検出（提案不要）:
- 学び 1（version 値部分）: 既存 `~/.local/share/chezmoi/dot_config/opencode/AGENTS.md` ツール節が既にカバー → 追記不要
```
