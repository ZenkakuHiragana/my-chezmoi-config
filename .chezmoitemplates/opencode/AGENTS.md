...
実質的な主張、計画、編集、レビュー指摘の前に、ユーザーの指示がどの作業段階に属するかと、そのまま次段階へ進めるかを確認し、解釈に必要な文脈を集める。作業段階や作業契約が未確定なら `context-clarification` で次段階へ進める条件を判定し、思い込みや前提の不足をなくす。
...
- 外部基盤文脈: フレームワーク、ランタイム、エンジン、OS、ハードウェア、外部ツールなど、ワークスペースを機能させるのに必要な前提知識。

### 文脈の充足条件

- 資料を読んだだけでは、文脈を確認済みとは扱わない。
- 各層は、今回の判断に必要な項目が埋まったときだけ確認済みとする。必要項目が埋まっていない場合は、資料を読んでいても不足として扱う。
- 依頼文脈では、目的、成果物、対象範囲、成功条件、制約、未決定事項の扱いを確認する。
- ワークスペース文脈では、このリポジトリやワークスペースの目的、主要機能、正とする情報源、今回の変更が全体目的にどう関係するかを確認する。
- サブシステム文脈では、変更対象の機能領域、主要ファイル、既存パターン、接続経路、呼び出し元、検証方法を確認する。
...
必要な層が `missing` または `blocked` のままなら、実装、レビュー、断定的な提案に進まない。
...
作業段階は状況の把握度合いに応じて次のように遷移する。遷移の前に、必要な前提を確認する。

> 診断 → 選択肢比較 → 方針決定 → (実装 or レビュー)
...
- 依頼を解釈するのに必要な資料を本当に読んだか確認する。
- 必要な事実、資料、対象範囲、確認に抜けがあるなら先へ進まない。
- 読んでいない資料を読んだことにしない。未確認は未確認と明記する。
