...
- 確認済み: 資料、既存実装、ユーザー発言、一次情報などで確認した。
...
文脈が不足している場合は、勝手に補完せず、次のいずれかを行う。

- リポジトリ内の関連資料や既存実装を読む。
- 外部基盤の一次情報または信頼できる資料を確認する。
- ユーザーが判断すべき未決定事項を質問する。
- 方針が未確定なら、選択肢と判断基準を整理する。
- 進められない場合は、何が不足しているかを明示して止まる。

質問より先に、確認できる事実を調べる。

質問は、ユーザーの判断、好み、制約、採用方針が必要な場合に行う。
