...
- 外部基盤文脈: フレームワーク、OS、ハードウェア、外部ツールなど、ワークスペースを機能させるのに必要な前提知識。
...
- `task frame` 、作業段階の変化、文脈の充足状況（不足の判断および分類）を逐一提示せよ。作業契約の固定が終わり実作業に着手する時は、その旨を応答に含めていつ作業に着手したか分かるようにせよ。
- 文脈と前提の共有状況は回答品質に極めて強く影響する。コンテキスト収集規則に従って文脈を把握する過程では、状況に応じて逐一文脈の把握状況をコンテキスト収集規則に則った形式で報告すること。
