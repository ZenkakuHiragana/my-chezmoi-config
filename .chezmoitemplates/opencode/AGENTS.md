...
実質的な主張、計画、編集、レビュー指摘の前に、ユーザーの指示がどの作業段階に属するかを確認し、解釈に必要な文脈を集める。
...
問題空間、判断基準、採用方針が未確定なら、実装段階として扱わない。
...
- サブシステム文脈: 変更対象が属する機能領域。主要ファイル、既存パターン、接続経路、検証方法。
...
- 確認済み: その層で今回の判断に必要な項目が、資料、既存実装、ユーザー発言、一次情報などで埋まっている。
- 不要: 今回の判断に関係しない理由を説明できる。
- 不足: 関係する可能性があるが、まだ確認できていない。

必要な層が不足のままなら、実装、レビュー、断定的な提案に進まない。
...
- ユーザー判断: 目的、成功条件、対象範囲、UX、見た目、互換性、破壊的変更、採用方針
- リポジトリ調査: ワークスペースの目的、正とする情報源、既存設計、既存実装、テスト、ログ
- サブシステム調査: 変更対象の主要ファイル、接続経路、呼び出し元、既存パターン、検証方法
- 外部基盤調査: フレームワーク、ランタイム、エンジン、OS、ハードウェア、外部ツールの仕様や制約
- 実装裁量: 低リスク、局所的、可逆で、既存パターンに従えば決められる実装詳細

不足した文脈の獲得における注意点は以下の通り。

- ユーザー判断に属する不足を、実装裁量として扱わない。
- 調査で解決できる不足は、質問より先に関連資料、既存実装、外部一次情報などを収集して調べる。
- 調査で解決できない不足は、質問、選択肢比較、推奨案の提示に進む。
- 実装裁量で決める場合は、既存パターンに従い、後から変更しやすい形にする。
