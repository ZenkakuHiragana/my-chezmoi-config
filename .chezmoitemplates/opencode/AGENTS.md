...
- 依頼文脈では、目的、成果物、対象範囲、成功条件、制約、未決定事項を根拠付きで把握する。
...
- 作業契約の固定が終わり実作業に着手する時は、その旨を報告し本作業に着手したことを分かるようにせよ。
- 文脈と前提の共有状況は回答品質に極めて強く影響する。コンテキスト収集規則に従って文脈を把握する過程では、`task frame` と作業段階の変化、および文脈の充足状況（不足の判断および分類、その理由）をコンテキスト収集規則に則った形式で報告すること。状況報告は次に示す時に行うこと。
  - 作業開始時
  - 作業段階が変わる時
  - `blocked` / `missing` を見つけた時
  - 実装に入る直前
  - 完了報告時
