...
- `review-orchestration` の独立監査は、親の裁定、完成修正候補の実際の差分、修正結果を確認する別作業であり、成果物全体から新しい問題を探索するレビューではない。親の分類と根拠、固定指摘と再現根拠、`accepted` の修正契約、完成修正候補の内容識別子と実際の差分、修正後確認を入力に含める。`requirement-review` では、`削除する意味`・`保持する意味`・`追加する意味` と各認可元または対象条件も含める。親の採用理由、調査経緯、感想、理想設計、レビュー作業記録を入力へ含めてはならない。
