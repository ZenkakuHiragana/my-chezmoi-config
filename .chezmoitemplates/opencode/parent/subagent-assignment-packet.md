...
- `review-orchestration` の修正前の新規独立確認は、親の裁定を監査する別作業であり、読者向け成果物のレビューではない。`requirement-review` の `accepted` を確認するときは、親の分類、再現根拠、`削除する意味`・`保持する意味`・`追加する意味` と各認可元または対象条件を入力に含める。親の採用理由、調査経緯、全体設計、コード差分、修正後成果物を入力へ含めてはならない。`code-review` と `japanese-doc-review` の `accepted` は今回の新しい確認対象に含めず、その他のレビュー種別へ要件レビュー用の意味差分入力を追加してはならない。
