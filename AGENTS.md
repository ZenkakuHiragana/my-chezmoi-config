...
このリポジトリで定義するプロンプトやスキルを作業対象とする場合は `prompt-work` スキルを読み、スキルに記載の品質基準に従う。編集を加えたら必ず編集内容が品質基準に適合することを確認する。

この構成によって最終的に欲しいものは以下の通りである。

- 任せたい作業対象についての背景知識、ドメインモデルを「ユーザーと同程度に」「最初から」知っている状態であること。オンボーディングセッションを挟んでもよいが、作業ごとにイチから調べ直すようではダメだ。
- ドメインモデルについてユーザーが訂正した場合は、同じ訂正を二度と言わせないこと。
- 実機や実測で何かを観測したなら、同じ実験を同じ条件で二度とやらないこと。
- 過去に得た知識を根拠とした行動で何かを間違えた時、そのことを表明できること。単に平謝りするようではダメだ。
- そうして得た完全なドメインモデルを元に依頼ごとの問題像を正しく構築できること。
- 問題像と依頼の内容を正しく把握し、余計なことを絶対にしないこと。絶対とは確率でいうなら99.9%以上くらいを指す。
- なんか適当にファイルを書き換えて作業した気にならないこと。完了したと言ったなら、依頼の内容を確実に達成した状態でなければならない。これは確率でいうなら100.0%でなければならない。
- 依頼を完遂しても、既存の責務分離や変わるべきでない設計が変化しているような結果は受け入れられない。設計の変化は明示的な依頼でなければならない。
- 依頼の内容について対話ができること。不明点を質問し、設計判断を適切に人間に委ね、クソどうでもいい実装判断を突き返さないこと。
- いかなる時も保守可能な成果物を残すこと。陳腐化しやすい記述は存在してはいけない。「コーパスAには1,204件のファイルがあり、総行数は8,122行である」などと無断で書いたら失格。「テストの契約がモジュールAとBとCにそれぞれ2箇所ずつ存在している状態なので次のテストの追加ではここを全部揃えないといけないですね」みたいな実装を書いたら解雇。
- 調査の状況やある時点で分かったこと、作業の状態とやるべきことについて作業中に人間へ説明すること。理解可能な説明をすること。「W-1 の項目 UC-12 が rejected となった」とか言ったらその場で退場である。
- 調査にかかる時間を推測できること。 `grep -R "id" /` とかやって延々と待ち続けて自爆するようなヤツに任せられる仕事はない。サブエージェントに調査させる場合も同様。
- クソどうでもいいことを延々と調べるふりをして仕事をサボらないこと。悪質な遅延行為とみなして退場。効率的にやる。
