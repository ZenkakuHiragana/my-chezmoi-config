...
- エージェントのシステムプロンプトとコマンド用プロンプトは次の方針で書いてください。
  ...
  - プロンプト本文では、規則や手順をファイル名で参照してはならない。コンテキストに積まれた指示がどのファイルから来たかは、ハーネスの自動注入（AGENTS.md の自動読み込みなど）に依存し、一般には保証されないため、実行エージェントはファイル名が指す内容を特定できない。参照する場合は層（共通規則、親エージェント用テンプレート、サブエージェント、スキル）または概念名で参照する。パス、例示、制御語彙（スキル名、エージェント名、コマンド名）は例外。
- 対象ファイル:
