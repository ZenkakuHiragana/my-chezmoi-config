...
- その論点へ実質的に触れた参加者だけを書く。`言及なし`の行を並べてはならない。
- 各箇条書きは結論を先に置き、1〜2文に収める。
- 同じ主張を複数の論点へ再掲してはならない。同じ回答を再掲する論点は一つへまとめる。
- `相違点` は同じ判断軸に対する違いだけを示す。違いがなければ `なし` と書き、作ってはならない。
- 資料ごとの要約を論点または相違点の代わりにしてはならない。
- 原回答全文を転載してはならない。要約で主張の意味、条件、確信度を変えてはならない。正しさ、順位、採否は判定しない。
- evidence file内部の`source_id`を返答へ出してはならない。根拠名が必要な場合はfile名または資料名を書く。

### 抜粋基準

論点別比較の後に置く。共通 `evidence packet` を作るときに適用した機械的な照合条件と抽出単位を、3項目以内で示す。
