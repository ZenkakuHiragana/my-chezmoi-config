...
ただし、validation が失敗した、または現行システムでの再発が確認された場合は別。
`covered_but_unvalidated` は、原則として編集ではなく退行検証への handoff にする。
...
- 公開調査、refactoring、文章作成、コードレビュー、investigation、検証の確立した実務が品質に影響する
...
- skill description に漏れた詳細手順
- 共有規則に漏れた局所例外
- role prompt や `SKILL.md` に置くべき詳細を共有規則が持っている箇所
...
- command 専用 agent プロンプト: 詳細な command 実行 role、handoff、artifact 契約、command 内の判断方針
...
- `likely_addressed` または `obsolete_context` の失敗記録から新規 prompt rule を作っていない
- 検証失敗または再発確認なしに `covered_but_unvalidated` の失敗記録を prompt 編集にしていない
...
意味のある能力について、正しく実装されたか判断できる validation target または同等の受け入れ確認が残っているか確認する。
