...
専用のローカル失敗ログがある場合は、今回の依頼または分類の引き渡しに関係する事例、分類レポート、まとまりだけ読む。
...
- 公開調査、リファクタリング、文章作成、コードレビュー、ローカル調査、検証の確立した実務が品質に影響する
...
- 共有規則: 短く安定した共有制約
- コマンド定義 / コマンド雛形: コマンドの入口、引数処理、コマンド固有の経路制御、短い作業契約
- コマンド専用エージェントプロンプト: 詳細なコマンド実行役割、引き渡し、成果物契約、コマンド内の判断方針
- 役割別プロンプトファイル: 1 つの役割、モード、エージェントに固有の動作
- スキル説明: 発見用の案内だけ。使う条件、使わない条件、期待される結果
...
影響を受けるコマンド、エージェントプロンプト、役割別プロンプト、スキル説明、`SKILL.md` について、作業種別に必要な最小要素を確認する。
