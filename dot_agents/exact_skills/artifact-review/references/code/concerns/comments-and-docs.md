...
この観点は、コメント、docstring、利用者向けメッセージ、埋め込みプロンプトが、コード、テスト、API、固定済み契約と同じ事実、制約、挙動を説明しているかを確認する。日本語の構成、文法、文体、誤記を代替レビューしてはならない。

日本語表現による誤読、条件反転、修飾範囲の曖昧さ、用語の区別の消失を確認する必要がある場合は、レビュー開始前に対象の日本語部分と固定済みの意味を特定し、`artifact-review` の文書分類による独立した検査単位を固定する。コードレビュー開始後に必要性が判明した場合は、現在の検査へ追加せず未確認事項として親エージェントへ返す。コードと文章が同じファイルにあっても、コード意味との一致と日本語表現を同じ検査項目へまとめてはならない。
...
- これはすぐ古くならないか。
- 成果物種別と主利用型は何か。内容は、その利用結果と読者が到達する条件に合っているか。
