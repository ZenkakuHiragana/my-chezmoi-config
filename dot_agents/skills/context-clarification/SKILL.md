...
意味を担う自然言語を確認した場合は、新しい専用表を増やさず、次の既存項目へ対応づける。

- `範囲`: 対象部分と接続面
- `達成すべき結果`: 文章として作成または構成し直す場合の成果物種別、主利用型、必要な利用結果
- `不変条件`: 保つ意味、意味の欠落やずれを判断する基準、利用主体が必要な結果へ到達する条件
- `条項根拠表`: 正本、情報の所有先、確認資料
- `受け入れ条件と確認`: 利用経路、観測する結果、確認主体
