...
現在タスクファイルは、継続依頼なのにタスクの識別子が会話から分からないときに限り見る。
現在依頼が明確なら無視する。
...
- レビュー結果へ対応するタスクでは、修正前の再現、まとめた修正、同じ確認方法による修正後確認を順に書く。
...
6. レビュー結果へ対応するタスクでは、修正前の再現、修正、修正後確認を一方向の作業項目として書く。
7. 作業項目を段階、依存関係、並列グループ、実行担当、副作用モード、読む範囲と書く範囲、成果物、確認方法付きで書く。
8. 完了前の確認を定義する。
...
- レビュー結果へ対応するタスクでは、修正前の再現が実装作業項目より前にある。
- レビュー結果へ対応するタスクでは、修正後に同じ確認方法と作業契約のテストを実行する。
- 現在タスクの識別子は短い識別子のみ。
- 次の能力集合が最小十分。
