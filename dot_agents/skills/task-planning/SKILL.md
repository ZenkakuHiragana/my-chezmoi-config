...
- レビュー指摘へ対応するtaskでは、`Review finding record`、有限な追試、`Review response contract`、閉じた対応後監査を分けて書く。
...
6. レビュー対応taskでは、有限な追試、1回の対応、閉じた対応後監査を一方向のwork itemとして書く。
...
- <None identified. または追試できた指摘をまとめた1 contract>
...
- レビュー対応taskでは、有限な追試と`Review response contract`が実装work itemより前にある。
- レビュー対応taskでは、対応後監査が閉じた質問だけを持ち、失敗時に縮小または巻き戻して終了する。
