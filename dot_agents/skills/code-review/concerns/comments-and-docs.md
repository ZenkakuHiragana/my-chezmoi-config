...
日本語表現による誤読、条件反転、修飾範囲の曖昧さ、用語の区別の消失を確認する必要がある場合は、レビュー開始前に対象の日本語部分と固定済みの意味を凍結し、`japanese-doc-review` の独立した検査単位を固定する。コードレビュー開始後に必要性が判明した場合は、現在の検査へ追加せず未確認事項として親へ返す。コードと文章が同じファイルにあっても、コード意味との一致と日本語表現を同じ検査項目へまとめてはならない。
