...
通常の `KNOWLEDGE.yml` には、環境に依存しない情報源の説明と到達方法を書く。ベース層の `instructions` は、他の設定ファイルや環境固有の実体を参照しなくても、その指示だけで正本へ到達できる手順であること。環境依存の値（ローカルパスなど）と環境固有の実体の利用方法（ローカルダンプ、キャッシュ、ナレッジベースなどを使う手順と優先順位）は `KNOWLEDGE.local.yml` の `instructions` で宣言する。ベースの `instructions` に、別の設定や環境固有の実体の存在を前提にした指示（「ローカル設定が指定する」「ローカルダンプを優先する」など）を書かない。`query_module` を持つ情報源では、モジュール固有の値を `query_options` に置ける。上書き後の情報源には source 名、`description`、`instructions` が必要である。
...
環境依存の値と環境固有の実体の利用方法は local 層で宣言する。ベース層のブロックは環境非依存の手順だけを書き、値を名前で参照する場合も、その名前が指す実体が無い環境で指示が成立すること。環境固有の実体の存在を前提にした命令（「〜が指定する」「〜を優先する」）をベース層に書かない。完全に異なる手順が必要な場合は、上書きを試みず別の source 名を定義する。
...
- ベース層の `instructions` だけで正本へ到達できるか。他の設定ファイルや環境固有の実体の存在を前提にした指示がないか。
- 環境固有の実体を使う手順と優先順位がベース層に書かれていないか（local 層に置くべきもの）。
