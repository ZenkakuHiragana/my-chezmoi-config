...
const sourceNamesSchema = z
  ...
  .min(1)
  .describe(
    "現在の統合済みカタログに存在する情報源名を1件以上指定する。メモをどの調査で検索するかを表す。自由なタグではない。",
  );
  ...
  const tools = [
    ...
    server.registerTool(
      "create_work_note",
      {
        title: "作業メモの作成",
        description:
          "人間が保存を明示的に承認した知見だけを新しい作業メモとして保存する。観測記録はその条件下で起きた事実として扱い、事実記述と矛盾する場合は観測結果を使う。観測は規範を変更しない。人間の承認前には呼び出さない。",
    ...
    server.registerTool(
      "update_work_note",
      {
        title: "作業メモの更新",
        description:
          "人間が更新を明示的に承認した既存の作業メモを更新する。呼出し前に read_work_note で現行全文を確認する。source_names は現行メモと同じ集合を指定し、対応する情報源は変更しない。人間の承認前には呼び出さない。",
        inputSchema: {
    ...
    server.registerTool(
      "read_work_note",
      {
        title: "作業メモの全文取得",
        description:
          "指定した現在有効な情報源に対応する作業メモを省略せず全文取得する。観測記録はその条件下で起きた事実として扱い、適用範囲を現在の状態へ照合する。",
        inputSchema: {
          source_name: z
            .string()
            .min(1)
