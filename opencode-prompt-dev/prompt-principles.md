...
- AGENTS.md、プロジェクト規則、ドメインメモ、ユーザー入力が特定領域のローカルリポジトリ、生成済みグラフ、runtime artifact、ログ、正本 path を示している場合、その領域に依存する質問では必要な根拠種別として扱う。
...
- ローカルの正本が必要な根拠種別に入った時点で、public-research は補助情報になり、ローカル根拠の調査を置き換えない。
