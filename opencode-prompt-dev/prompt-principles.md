...
- review対象は役割で分ける。仕様と作業契約は`requirement-review`、codeとdiffは`code-review`、利用者向け日本語本文は`japanese-doc-review`を使い、別の対象を代替してはならない。
- reviewerは対象、観点、場所、破綻、根拠、同じ入力から再現できる確認方法を返す。完全化、好み、一般論を修正要求にしてはならない。
- review結果へ対応する前に確認方法を修正前の対象へ適用し、失敗を再現する。再現できない問題を変更理由にしてはならない。
- 対応後は同じ確認方法とtask contractのtestsを実行する。失敗する変更は縮小または巻き戻し、完了扱いにしない。
