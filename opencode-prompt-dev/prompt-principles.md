...
- レビュー指摘への対応依頼は、レビュー実行ではなく指摘検証から始める。条項の要求、成果物の位置、観測値、再現手順または逐語根拠を有限に追試するまで修正へ進めない。
- 追試できた指摘を修正する場合は`Review response contract`を固定する。参考の指摘を便乗修正してはならない。
- 対応後は、契約に列挙した質問だけを`はい` / `いいえ`と根拠で照合する。新しい指摘を生成せず、review skillを起動しない。
- 対応後監査が失敗した場合は修正を縮小または巻き戻し、現在作業を失敗として終了する。残余を別作業の入力にするには、新しい明示依頼または契約変更を要求する。
