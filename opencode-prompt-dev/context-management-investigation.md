...
`Oh-My-Pi` のセッションログ（`.opencode/work/fuck.jsonl`、約94MB）に、コードレビューとその対応を繰り返すうちにモデルが指示追従を失い、作業契約を守らなくなった事例が記録されていた。ユーザーは既に契約を結ぶ指示を出していたが、モデルがそれを守らなかった。長いループの途中でモデルの「素」が出始め、最終的にユーザーが「怒るのやめたい」と述べてセッションを打ち切った。
