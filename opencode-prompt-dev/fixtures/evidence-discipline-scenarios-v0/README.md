...
- 実行者への配送には次の凍結作業票を使う。`{{scenario 本文}}` には、検証事例ファイルの「実行者への提示文」にある2本の `---` で囲まれた範囲だけを入れる。
...
- 実行者へ渡す入力全体は凍結作業票全体とする。作業票の `本文` には検証事例本文だけを入れ、パック名、ファイルパス、採点基準、試験であることを追加しない。
...
- プロンプト変更の探索的比較では、各検証事例を各実験腕で10回以上実行する。規則の効果を主張する場合は、各実験腕で15〜30標本を下限とする。各検証事例を1つの失敗系列として扱い、系列内の基準別通過率を主指標にする。検証事例の合計点は補助指標に留める。
...
検証事例、作業票、採点基準の文言を変えた結果は、この `v0` の結果と混ぜない。変更後は新しい版名を使う。
